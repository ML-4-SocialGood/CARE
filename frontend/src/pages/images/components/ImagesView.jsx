...
export default function ImagesView({ detects, label, confLow, confHigh }) {
  ...
  useEffect(() => {
    ...
    const fetchFiles = async () => {
      ...
      const response = await apiClient(
        `/api/users/detect_images/browse?date=${date}&folderPath=${paths.join("/")}${label ? `&label=${label}` : ""}&confLow=${confLow}&confHigh=${confHigh}`,
        {
          method: "GET",
          headers: {
  ...
  const handleSelectAll = async () => {
    const response = await apiClient(
      `/api/users/detect_images/select_all?path=${currentFolder}${label ? `&label=${label}` : ""}&confLow=${confLow}&confHigh=${confHigh}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
  ...
  const handleDownload = async () => {
    try {
      const response = await apiClient('/api/users/detect_images/download_selected', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
...
function PaginateItems({
  ...
  return (
    <>
      ...
      <Button 
        className="downloadSelected-button" 
      ...
      {/* "ReID Selected" button always visible */}
      <Button 
  ...
  if (status === statuses.processing) {
    return (
      <>
        ...
        <p>
          The Re-Identification AI model is processing your selected images.
          This is a long-running task. <br />
          A progress bar will appear if the AI server successfully receives your images. 
