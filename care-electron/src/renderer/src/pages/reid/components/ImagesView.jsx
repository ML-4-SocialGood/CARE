...
export default function ImagesView({ detects: initialDetects }) {
  ...
  /*const onDelete = async (itemId) => {
    ...
    if (window.confirm(`Are you sure you want to delete the record for date ${date} and time ${time}?`)) {
      try {
        ...
        if (response.ok) {
          ...
          setFiles((prevFiles) => {
            const updatedFiles = prevFiles.filter(file => file.path !== itemId);
            console.log("Updated files after deletion:", updatedFiles);
            return updatedFiles;
          });
          // Delete the folder from the folder tree
          fetchFiles();
          alert(`Successfully deleted the record for date ${date} and time ${time}.`);
        } else {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setDetects((prevFiles) => {
          ...
          const hasOtherChildren = updatedFiles.some(file => file.path.startsWith(`${parentPath}/`));

          // Remove the parent path if it has no other children
          const finalFiles = hasOtherChildren ? updatedFiles : updatedFiles.filter(file => file.path !== parentPath);
    ...
    if (newID) {
      try {
        ...
        if (response.ok) {
          ...
          if(newID === oldGroupId){
            setNotificationMessage("The new name is the same as the old name. The group name will not change.");
            setShowNotificationModal(true);
          } else{
